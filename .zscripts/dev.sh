...
log_step_start "ensure yt-dlp"
echo "[SETUP] Checking yt-dlp..."
VENV_BIN="${HOME:-/root}/.venv/bin"
if ! command -v yt-dlp &>/dev/null && [ ! -f "$VENV_BIN/yt-dlp" ]; then
    echo "[SETUP] yt-dlp not found, installing..."
    pip3 install yt-dlp 2>/dev/null || true
fi
if [ -f "$VENV_BIN/yt-dlp" ]; then
    echo "[SETUP] yt-dlp OK: $($VENV_BIN/yt-dlp --version 2>/dev/null | head -1)"
else
    echo "[SETUP] WARNING: yt-dlp still not found, will auto-install on first request"
fi
log_step_end "ensure yt-dlp"
